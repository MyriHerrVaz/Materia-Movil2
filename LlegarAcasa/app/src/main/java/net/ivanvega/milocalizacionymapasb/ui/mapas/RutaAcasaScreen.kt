...
fun CurrentLocationContent(usePreciseLocation: Boolean) {
    ...
    var homeMyriam = remember {
        LatLng(20.098581847165367, -101.39231694032628)
    }

    var worked = remember {
        LatLng(20.105157448519957, -101.38948360175382)
    }
    var Itsur = remember {
    ...
    Column(
        ...
        val markerState = rememberMarkerState(position = worked)
        val markerState2 = rememberMarkerState(position = homeMyriam )
        ...
        Box(Modifier.fillMaxSize()) {
            GoogleMap(
                ...
                // point trabajo
                Marker(
                    state = MarkerState(position = worked),
                    title = "Trabajo",
                    snippet = "Trabajo"
                )
                // point Casa
                Marker(
                    state = MarkerState(position = homeMyriam),
                    title = "Casa de Jovany",
                    snippet = "Casa"
                )

                //verifica la posicion actual con la casa de myriam
                if (!markerState.position.equals(homeMyriam)) {
                    Marker(
                        state = MarkerState(position = markerState.position),
                        title = "Ubicacion actual"
                    )
                    val RouteList = remember { mutableStateOf<List<LatLng>>(emptyList()) }
                    createRoute(homeMyriam, markerState.position) { routePoints ->
                ...
                //lo mismo pero ahora con el trabajo
                } else if (!markerState2.position.equals(worked)) {
                    Marker(
                        state = MarkerState(position = markerState2.position),
                        title = "Ubicacion actual"
                    )
                    val RouteList = remember { mutableStateOf<List<LatLng>>(emptyList()) }
                    createRoute(worked, markerState2.position) { routePoints ->
                        val pointsList = mutableListOf<LatLng>()
                        for (i in routePoints.indices step 2) {
                            val lat = routePoints[i]
                            val lng = routePoints[i + 1]
